...
/**
  ...
  * @param msgArgs any arguments expected by the message in the conf/messages file
  */
case class EmbeddedMessage(msgKey: String, msgArgs: MessageArguments = MessageArguments()) extends MessageLookup
...
/**
  ...
  */
case class SummaryError(msgKey: String, msgArgs: MessageArguments = MessageArguments(), anchor: String) extends MessageLookup {
  def this(msgKey: String, anchor: String) = this(msgKey, MessageArguments(), anchor)
...
object SummaryError {
  def apply(msgKey: String, anchor: String): SummaryError =
    new SummaryError(msgKey, anchor)

  def apply(fieldError: FieldError, anchor: String): SummaryError =
    new SummaryError(ErrorMessageInterpreter.defaultSummaryId(fieldError.msgKey), anchor)

  def apply(fieldError: FieldError, summaryArgs: MessageArguments, anchor: String): SummaryError =
    new SummaryError(ErrorMessageInterpreter.defaultSummaryId(fieldError.msgKey), summaryArgs, anchor)
}
...
/**
  ...
  * @param msgArgs any arguments expected by the message in the conf/messages file
  */
case class FieldError(msgKey: String, msgArgs: MessageArguments = MessageArguments()) extends MessageLookup {
  override def hashCode(): Int = {
    var code = this.productPrefix.hashCode()
    val arr = this.productArity
    var i = 0
